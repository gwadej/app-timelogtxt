...
a time/date stamp and an event.
...
To enhance reporting, there are two forms of formatted tags. The first string
...
If no task is specified, all of the event except the project is treated as
the task. The task is optional because not everyone will find it useful to
...
The timelog.txt file would contain this string with no leading or trailing
whitespace. The project in this case is C<Timelog>, which specifies that
I am working on this module. The task is C<Docment>, which in means I am
documenting something on the project. The string 'File Format' specifies more
detail on what I am documenting.

This event began around 10:01 on July 1st.

=head3 Event With No Detail

This following event is also relatively complete, except there was no need for
more detail
